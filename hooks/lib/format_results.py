...
def format_results(response_file, project_dir=None):
    ...
    lines = [
        ...
        "IMPORTANT: Check these results for known bugs ([OPEN], [CLOSED·not_planned]) and workarounds that affect the nodes you're recommending. If a node or approach has a known issue, design around it — use the workaround in your answer instead of suggesting the broken path. Cite the issue number when relevant.",
